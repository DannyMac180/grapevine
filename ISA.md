...
- D-11 (2026-08-08, iteration 2 — marketplace distribution): Repo made its own Claude Code plugin marketplace (`.claude-plugin/marketplace.json`, marketplace name = plugin name = `grapevine` → install string `grapevine@grapevine`). README install section split into marketplace (recommended) + local-clone paths; stale `danmcateer/grapevine` URL corrected to `DannyMac180/grapevine` (repo-wide sweep clean). Validated with `claude plugin validate` (pass) and live raw.githubusercontent probes post-push (main = default branch). Advisor's remaining ask — a true `/plugin marketplace add` → `/plugin install` loop from a clean session — is interactive-only: [DEFERRED-VERIFY] follow-up for Dan's next session.
